...
(() => {
  ...
  // ---------- フロー横断一覧 ----------
  function flowOrder(map,allowedIds=null) {
    const source=allowedIds?map.nodes.filter(n=>allowedIds.has(n.id)):map.nodes;
    const sorted=source.slice().sort((a,b)=>a.y-b.y||a.x-b.x),incoming=new Set(),sourceIds=new Set(source.map(n=>n.id));
    source.forEach(n=>{if(n.targetId&&sourceIds.has(n.targetId))incoming.add(n.targetId);n.options.forEach(o=>{if(o.targetId&&sourceIds.has(o.targetId))incoming.add(o.targetId)})});
    const root=sorted.find(n=>!incoming.has(n.id))||sorted[0];if(!root)return[];
    const byId=new Map(source.map(n=>[n.id,n])),visited=new Set(),result=[],queue=[root];
  ...
  function flowComponentOrders(map){if(!map.nodes.length)return[[]];const sorted=map.nodes.slice().sort((a,b)=>a.y-b.y||a.x-b.x),adj=new Map(sorted.map(n=>[n.id,new Set()]));map.nodes.forEach(n=>{const targets=n.options.length?n.options.map(o=>o.targetId):[n.targetId];targets.forEach(id=>{if(!id||!adj.has(id))return;adj.get(n.id).add(id);adj.get(id).add(n.id)})});const seen=new Set(),orders=[];sorted.forEach(start=>{if(seen.has(start.id))return;const ids=new Set(),queue=[start.id];while(queue.length){const id=queue.shift();if(seen.has(id))continue;seen.add(id);ids.add(id);adj.get(id).forEach(next=>{if(!seen.has(next))queue.push(next)})}orders.push(flowOrder(map,ids))});return orders;}
  function flowSummary(map,order,componentIndex=0,componentCount=1){const first=order[0]||null,next=order.find(n=>n.status!=='done')||null,focus=next||first,stateInfo=focus?(next?getDisplayState(focus):{key:'done',label:'処理済み'}):{key:'done',label:'項目なし'};return{map,order,componentIndex,componentCount,key:`${map.id}::${first?.id||'empty'}`,first,next,focus,stateInfo,startDate:focus?.startDate||'',dueDate:focus?.dueDate||''};}
  function allFlowSummaries(){return Store.maps.flatMap(map=>{const orders=flowComponentOrders(map);return orders.map((order,index)=>flowSummary(map,order,index,orders.length))});}
  ...
  // ---------- 実行モード ----------
  function startRun(){const map=Store.current();if(!map)return;const start=getNode(state.selectedNodeId)||map.nodes[0];if(!start){toast('実行する項目がありません');setMode('edit');return;}state.runNodeId=start.id;state.runSteps=0;state.runPath=[start.id];refs.runPanel.hidden=false;renderRun();}
  ...
  function startListPrint(){state.printMode='list';state.printSelection=new Set(allFlowSummaries().map(row=>row.key));showPrintBar('印刷するものにチェック','デフォルトですべて選択されています。不要なフローのチェックを外してください。','チェックしたフローを印刷');renderFlowList();updateListPrintCount();}
  function decorateListPrint(){refs.flowList.querySelectorAll('.flow-list-card').forEach(card=>{card.classList.add('print-choice');const label=document.createElement('label');label.className='list-print-check';label.title='印刷する';label.innerHTML=`<input type="checkbox" ${state.printSelection.has(card.dataset.flowKey)?'checked':''} aria-label="このフローを印刷">`;label.querySelector('input').onchange=e=>{e.target.checked?state.printSelection.add(card.dataset.flowKey):state.printSelection.delete(card.dataset.flowKey);updateListPrintCount()};card.prepend(label)});}
  ...
  function printSelectedFlows(){const rows=allFlowSummaries().filter(row=>state.printSelection.has(row.key)).sort(compareFlow);refs.printSheet.innerHTML=`<div class="print-list-document"><h1 class="print-document-title">カスタムフローチャート フロー一覧</h1>${rows.map(r=>`<article class="print-list-row"><h2>${escapeHtml(r.map.name)}${r.componentCount>1?`（別フロー ${r.componentIndex+1}/${r.componentCount}）`:''}</h2><p><b>フローの最初：</b>${escapeHtml(r.first?.title||'項目なし')}</p><p><b>次に対応：</b>${escapeHtml(r.next?.title||'すべて処理済み')}（${r.stateInfo.label}）</p><p>開始 ${r.focus?fullDate(r.focus.startDate):'—'}　／　完了期限 ${r.focus?fullDate(r.focus.dueDate):'—'}</p></article>`).join('')}</div>`;finishPrintSelection();sendToPrinter();}
