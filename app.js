...
(() => {
  ...
  const state = { mode: 'edit', listSort: 'status', selectedNodeId: null, zoom: 1, panX: 20, panY: 30, mobileMapId: null, history: [], future: [], activeEdge: null, connectionDraft: null, runNodeId: null, runSteps: 0, runPath: [], printMode: null, printOrigin: null, printSelection: new Set() };
  ...
  const refs = {
    app: $('#app'), workspace: $('#workspace'), listPage: $('#listPage'), flowList: $('#flowList'), listSummary: $('#listSummary'), listSort: $('#listSort'), viewport: $('#viewport'), world: $('#world'), nodeLayer: $('#nodeLayer'), edgeLayer: $('#edgeLayer'), empty: $('#emptyState'),
    ...
    mapList: $('#mapList'), templateList: $('#templateList'), newMapName: $('#newMapName'), zoomReset: $('#zoomResetButton'), toast: $('#toast'), runPanel: $('#runPanel'), runCard: $('#runCard'),
  ...
  function openFlowFromList(mapId,nodeId){Store.currentId=mapId;Store.save();state.mode='edit';state.selectedNodeId=nodeId||null;state.history=[];state.future=[];renderAll();const node=getNode(nodeId);if(node){state.panX=refs.viewport.clientWidth/2-(node.x+120)*state.zoom;state.panY=refs.viewport.clientHeight/2-(node.y+60)*state.zoom;updateTransform();}}
  ...
  function renderNodes() {
    ...
    nodes.forEach(node => {
      ...
      el.addEventListener('click', event => {
        ...
        if (state.mode === 'edit') { state.selectedNodeId = node.id; renderAll(); }
  ...
  function edgePath(a, b, key = 'main') {
    ...
    if (endY >= startY - 20) return { d: `M ${startX} ${startY} C ${startX} ${startY + distance}, ${endX} ${endY - distance}, ${endX} ${endY}`, labelX: (startX + endX) / 2, labelY: (startY + endY) / 2 };
    const side = startX <= endX ? 1 : -1; const bendX = startX + side * 170;
    return { d: `M ${startX} ${startY} C ${bendX} ${startY + 40}, ${bendX} ${endY - 40}, ${endX} ${endY}`, labelX: bendX, labelY: (startY + endY) / 2 };
  }
  function estimateHeight(n) { const el=refs.nodeLayer.querySelector(`[data-id="${n.id}"]`);return el?.offsetHeight||102; }
  function renderEdges() {
    refs.edgeLayer.replaceChildren(); const nodes = Store.current()?.nodes || []; const byId = new Map(nodes.map(n => [n.id,n]));
    nodes.forEach(from => {
      ...
      links.forEach(link => { const to=byId.get(link.targetId); if (!to) return; const p=edgePath(from,to,link.key); const active=state.activeEdge && state.activeEdge.from===from.id && state.activeEdge.key===link.key;
        ...
        if (link.label) { const width=Math.min(180,Math.max(44,link.label.length*13+20)); const rect=svg('rect',{x:p.labelX-width/2,y:p.labelY-13,width,height:26,class:'edge-label-bg'}); const text=svg('text',{x:p.labelX,y:p.labelY+1,class:'edge-label'}); text.textContent=link.label; refs.edgeLayer.append(rect,text); }
      });
    });
  ...
  function renderInspector() {
    ...
    refs.inspector.classList.toggle('mobile-open', !!node);
  ...
  function beginNodeDrag(event){if(state.mode!=='edit'||event.button!==0||event.target.closest('.node-port'))return;event.stopPropagation();const el=event.currentTarget,node=getNode(el.dataset.id);if(!node)return;const startX=event.clientX,startY=event.clientY,originX=node.x,originY=node.y;let moved=false,recorded=false;el.setPointerCapture(event.pointerId);
    const move=e=>{const dx=(e.clientX-startX)/state.zoom,dy=(e.clientY-startY)/state.zoom;if(Math.abs(dx)+Math.abs(dy)>5){moved=true;if(!recorded){snapshot();recorded=true;}node.x=Math.max(0,Math.round(originX+dx));node.y=Math.max(0,Math.round(originY+dy));el.style.left=`${node.x}px`;el.style.top=`${node.y}px`;renderEdges();}};
    const up=()=>{el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up);el.removeEventListener('pointercancel',up);if(moved){el.dataset.dragged='true';node.updatedAt=now();Store.current().updatedAt=now();Store.save();}};el.addEventListener('pointermove',move);el.addEventListener('pointerup',up);el.addEventListener('pointercancel',up);
  }
  ...
  function beginConnectionDrag(event){if(state.mode!=='edit'||event.button!==0)return;event.preventDefault();event.stopPropagation();const port=event.currentTarget,card=port.closest('.flow-node'),from=getNode(card.dataset.id),key=port.dataset.key;if(!from)return;const currentTarget=key==='main'?from.targetId:from.options.find(o=>o.id===key)?.targetId;card.classList.add('connecting-source');document.querySelectorAll('.input-port').forEach(p=>{if(p.closest('.flow-node')?.dataset.id!==from.id)p.classList.add('connection-target')});state.connectionDraft={fromId:from.id,key,pointer:clientToWorld(event.clientX,event.clientY)};renderEdges();port.setPointerCapture(event.pointerId);
    const move=e=>{state.connectionDraft.pointer=clientToWorld(e.clientX,e.clientY);renderEdges()};
    const up=e=>{port.removeEventListener('pointermove',move);port.removeEventListener('pointerup',up);port.removeEventListener('pointercancel',cancel);card.classList.remove('connecting-source');document.querySelectorAll('.input-port').forEach(p=>p.classList.remove('connection-target'));const targetEl=document.elementFromPoint(e.clientX,e.clientY)?.closest('.input-port'),targetId=targetEl?.closest('.flow-node')?.dataset.id;state.connectionDraft=null;if(targetId&&targetId!==from.id){if(targetId!==currentTarget){snapshot();setLinkTarget(from,key,targetId);commit('項目を接続しました')}else renderEdges();}else if(currentTarget){snapshot();setLinkTarget(from,key,null);commit('接続を外しました')}else renderEdges();};
  ...
  function setMode(mode){if(state.printMode)finishPrintSelection();state.mode=mode;if(mode==='run'){startRun();return;}refs.runPanel.hidden=true;state.selectedNodeId=null;renderAll();}
  ...
  function printCurve(start,end){const distance=Math.max(55,Math.abs(end.y-start.y)*.45);return`M ${start.x} ${start.y} C ${start.x} ${start.y+distance}, ${end.x} ${end.y-distance}, ${end.x} ${end.y}`;}
  function renderDiagramPrint(idSource,title,closeEditSelection=false){const ids=new Set(idSource),map=Store.current(),nodes=map.nodes.filter(n=>ids.has(n.id));if(!nodes.length){toast('印刷する項目を選んでください');return false;}const sizes=new Map(nodes.map(n=>{const el=refs.nodeLayer.querySelector(`[data-id="${n.id}"]`),w=el?.offsetWidth||((n.options.length)?280:240),h=el?.offsetHeight||Math.max(110,105+Math.ceil(n.options.length/2)*42);return[n.id,{w,h,el}]})),minX=Math.min(...nodes.map(n=>n.x)),minY=Math.min(...nodes.map(n=>n.y)),maxX=Math.max(...nodes.map(n=>n.x+sizes.get(n.id).w)),maxY=Math.max(...nodes.map(n=>n.y+sizes.get(n.id).h)),pad=70,w=maxX-minX+pad*2,h=maxY-minY+pad*2,scale=Math.min(1.3,960/w,580/h);refs.printSheet.replaceChildren();const page=document.createElement('div');page.className='print-flow-page';const heading=document.createElement('h1');heading.className='print-flow-title';heading.textContent=title;page.appendChild(heading);const stage=document.createElement('div');stage.className='print-flow-stage';stage.style.width=`${w}px`;stage.style.height=`${h}px`;stage.style.transform=`translate(-50%,-46%) scale(${scale})`;const lines=svg('svg',{width:w,height:h});const defs=svg('defs'),marker=svg('marker',{id:'print-arrow',markerWidth:9,markerHeight:7,refX:8,refY:3.5,orient:'auto'});marker.appendChild(svg('path',{d:'M0,0 L9,3.5 L0,7 Z',fill:'#8b98a9'}));defs.appendChild(marker);lines.appendChild(defs);nodes.forEach(from=>{const links=from.options.length?from.options.map(o=>({targetId:o.targetId,label:o.label})):[{targetId:from.targetId,label:''}];links.forEach(link=>{const to=map.nodes.find(n=>n.id===link.targetId);if(!to||!ids.has(to.id))return;const fs=sizes.get(from.id),ts=sizes.get(to.id),start={x:from.x-minX+pad+fs.w/2,y:from.y-minY+pad+fs.h},end={x:to.x-minX+pad+ts.w/2,y:to.y-minY+pad},d=printCurve(start,end),path=svg('path',{d,class:'edge','marker-end':'url(#print-arrow)'});lines.appendChild(path);if(link.label){const x=(start.x+end.x)/2,y=(start.y+end.y)/2,width=Math.max(44,Math.min(180,link.label.length*13+20));lines.appendChild(svg('rect',{x:x-width/2,y:y-13,width,height:26,class:'edge-label-bg'}));const text=svg('text',{x,y:y+1,class:'edge-label'});text.textContent=link.label;lines.appendChild(text)}})});stage.appendChild(lines);nodes.forEach(n=>{const card=printableCard(n);card.classList.remove('selected','print-picked','highlighted');card.style.left=`${n.x-minX+pad}px`;card.style.top=`${n.y-minY+pad}px`;stage.appendChild(card)});page.appendChild(stage);refs.printSheet.appendChild(page);if(closeEditSelection)finishPrintSelection();sendToPrinter();return true;}
  ...
  function bindEvents(){
    ...
    $('#addOptionButton').onclick=addOption;$('#deleteNodeButton').onclick=deleteNode;$('#duplicateNodeButton').onclick=duplicateNode;$('#closeInspector').onclick=()=>{state.selectedNodeId=null;renderAll()};
