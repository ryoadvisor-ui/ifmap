...
(() => {
  ...
  function cubicGeometry(x1,y1,c1x,c1y,c2x,c2y,x2,y2){return{d:`M ${x1} ${y1} C ${c1x} ${c1y}, ${c2x} ${c2y}, ${x2} ${y2}`,labelX:(x1+3*c1x+3*c2x+x2)/8,labelY:(y1+3*c1y+3*c2y+y2)/8};}
  ...
  function beginNodeDrag(event){if(state.mode!=='edit'||event.button!==0||event.target.closest('.node-port')||pinchSequencePointers.has(event.pointerId)||activeNodeDragPointer!==null)return;event.stopPropagation();const el=event.currentTarget,node=getNode(el.dataset.id);if(!node)return;activeNodeDragPointer=event.pointerId;const startX=event.clientX,startY=event.clientY,originX=node.x,originY=node.y;let moved=false,recorded=false;el.setPointerCapture(event.pointerId);
    const move=e=>{if(pinchSequencePointers.has(e.pointerId))return;const dx=(e.clientX-startX)/state.zoom,dy=(e.clientY-startY)/state.zoom;if(Math.abs(dx)+Math.abs(dy)>5){moved=true;if(!recorded){snapshot();recorded=true;}node.x=Math.max(0,Math.round(originX+dx));node.y=Math.max(0,Math.round(originY+dy));el.style.left=`${node.x}px`;el.style.top=`${node.y}px`;renderEdges();}};
    const up=()=>{el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up);el.removeEventListener('pointercancel',up);if(activeNodeDragPointer===event.pointerId)activeNodeDragPointer=null;if(moved){el.dataset.dragged='true';node.updatedAt=now();Store.current().updatedAt=now();Store.save();}};el.addEventListener('pointermove',move);el.addEventListener('pointerup',up);el.addEventListener('pointercancel',up);
  }
